...
public class Bag {
    ...
    public Bag intersection(Bag bag) {
        Bag result = new Bag();
        for (BagItem bi : bag.data) {
            for (BagItem bi2 : this.data) {
                if(bi.getItem().equals(bi2.getItem())) {
                    result.add(bi.getItem(), bi.getNum() + bi2.getNum());}
        }}
        return result;
    }
    ...
    public Bag difference(Bag bag) {
        ...
        for (BagItem bi2 : bag.data) {
            result.remove(bi2.getItem(), bi2.getNum());
        }
        return result;
    }
